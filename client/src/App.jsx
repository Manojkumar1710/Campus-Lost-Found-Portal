...
function App() {
  ...
  const go = (nextView) => { setView(nextView); setSelectedListing(null); setMobileMenuOpen(false); window.scrollTo({ top: 0, behavior: "smooth" }); };
  const requireAuth = (nextView) => { setMobileMenuOpen(false); if (!token) setAuthMode("login"); else go(nextView); };
  ...
  return <div className="app-shell">
    <header className="topbar"><button className="brand" type="button" onClick={() => go("home")}><span className="brand-mark">LF</span><span><strong>CampusFind</strong><small>Lost & found, together</small></span></button><nav className={mobileMenuOpen ? "open" : ""} aria-label="Primary navigation"><button className={view === "home" || view === "browse" ? "active" : ""} type="button" onClick={() => go("home")}>Discover</button><button className={view === "report" ? "active" : ""} type="button" onClick={() => requireAuth("report")}>Report an item</button><button className={view === "activity" ? "active" : ""} type="button" onClick={() => requireAuth("activity")}>My reports</button></nav><div className="topbar-actions"><button className="account-button" type="button" onClick={() => token ? go("account") : setAuthMode("login")}>{token ? <span className="avatar">{initials(user?.name)}</span> : null}<span>{token ? user?.name?.split(" ")[0] : "Sign in"}</span></button><button className="menu-toggle" type="button" aria-label={mobileMenuOpen ? "Close navigation" : "Open navigation"} aria-expanded={mobileMenuOpen} onClick={() => setMobileMenuOpen((open) => !open)}><span></span><span></span><span></span></button></div></header>
    <main>
